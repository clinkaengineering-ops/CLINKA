...
const paymob_webhook_1 = require("./modules/payments/paymob.webhook");
async function runTests() {
    ...
    const isReplayed = (0, paymob_webhook_1.isWebhookReplayed)(oldDate, 5);
    if (isReplayed) {
    ...
    const isReplayedValid = (0, paymob_webhook_1.isWebhookReplayed)(validDate, 5);
    if (!isReplayedValid) {
    ...
    const validHmac = node_crypto_1.default.createHmac("sha512", hmacSecret).update(payloadString).digest("hex");
    const validHmacPrev = node_crypto_1.default.createHmac("sha512", hmacSecretPrev).update(payloadString).digest("hex");
    ...
    if (!(0, paymob_webhook_1.verifyPaymobPayoutHmac)(payload, tamperedHmac, secrets)) {
    ...
    console.log("\n[Test 3] Secret Rotation Verification");
    if ((0, paymob_webhook_1.verifyPaymobPayoutHmac)(payload, validHmac, secrets)) {
        console.log("✅ SUCCESS: Current HMAC Secret accepted");
    }
    else {
        console.error("❌ FAIL: Current HMAC Secret rejected");
    }
    if ((0, paymob_webhook_1.verifyPaymobPayoutHmac)(payload, validHmacPrev, secrets)) {
