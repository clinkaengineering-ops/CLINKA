...
async function prepareProjectCheckoutSession(clientId, projectId) {
    const project = await db_1.default.project.findUnique({
        where: { id: projectId },
        include: { payment: true, client: true },
    });
...
async function releaseEscrowPayment(clientId, paymentId) {
    ...
    await createNotification(payment.engineerId, "FUNDS_RELEASED", "Funds released", `Escrow for "${projectTitle}" was released to you`, `/escrow`);
    return updated;
}
