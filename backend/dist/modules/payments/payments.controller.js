...
async function getCheckoutSessionController(req, res, next) {
    try {
        ...
        const session = await (0, payments_service_1.prepareProjectCheckoutSession)(req.user.userId, projectId);
        res
            .status(200)
            .json((0, ApiResponse_1.default)(200, "Checkout session ready", session));
    }
    catch (error) {
        next(error);
    }
...
async function listEscrowController(req, res, next) {
    try {
        ...
        res.status(200).json((0, ApiResponse_1.default)(200, "Escrow items fetched successfully", items));
    }
    catch (error) {
        next(error);
    }
