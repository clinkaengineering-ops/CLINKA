...
async function createBid(engineerId, projectId, data) {
    await (0, ban_service_1.assertUserNotBanned)(engineerId, "place bids");
...
async function approveBid(clientId, bidId) {
    ...
    if (engineerUser) {
        await (0, notifications_1.createNotification)(engineerUser.id, "BID_ACCEPTED", "Bid accepted", `Your bid on "${project.title}" was accepted. The client will pay to start — you'll get a message when payment is received.`, `/messages?project=${project.id}`);
    }
    return { message: "Bid approved and project assigned to engineer" };
}
async function listMyBids(engineerUserId) {
    await (0, ban_service_1.assertUserNotBanned)(engineerUserId, "view your bids");
    const profile = await db_1.default.engineerProfile.findUnique({
