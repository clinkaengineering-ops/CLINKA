import {
  ...
  type TextareaHTMLAttributes, forwardRef
} from "react";
...
export const Input = forwardRef<HTMLInputElement, InputHTMLAttributes<HTMLInputElement> & { icon?: ReactNode }>(
  ({ className, icon, ...props }, ref) => (
    <div className="relative">
      ...
      <input
        ref={ref}
        className={cn("w-full h-10 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-electric-500/30 transition", icon && "pl-10", className)}
        {...props}
      />
    </div>
  )
...
export const VerifiedBadge = ({ size = 16 }: { size?: number }) => (
  <span className="inline-flex items-center justify-center bg-electric-500 rounded-full text-white" style={{ width: size, height: size }}>
  ...
  <Card className="p-5 hover:border-electric-500/40 transition group">
    <div className="flex items-start justify-between">
      <div>
        ...
        {change && <p className={cn("mt-1 text-xs font-medium", accent === "down" ? "text-rose-500" : "text-emerald-500")}>{accent === "down" ? "↓" : "↑"} {change}</p>}
      </div>
      <div className="h-10 w-10 rounded-xl bg-electric-500/10 text-electric-600 dark:text-electric-400 flex items-center justify-center group-hover:scale-110 transition">{icon}</div>
    </div>
  </Card>
);

export const Field = ({ label, right, children }: { label: string; right?: ReactNode; children: ReactNode }) => (
  <div>
