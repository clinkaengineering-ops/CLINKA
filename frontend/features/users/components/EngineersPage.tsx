...
export function EngineersPage() {
  ...
  const disciplines = [
    ...
    { id: "ARCHITECTURAL", label: t("disc.architecture") },
  ];

  const filtered = engineers.filter(e => {
    const matchSearch = e.name.toLowerCase().includes(search.toLowerCase());
    const matchSpecialty = active === "All" || e.profile?.specialty === active;
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {loading ? (
        <div className="grid sm:grid-cols-2 gap-5">
          {[...Array(4)].map((_, i) => <EngineerCardSkeleton key={i} />)}
        </div>
      ) : error ? (
        ...
        <div className="grid sm:grid-cols-2 gap-5">
          {filtered.map(e => <EngineerCard key={e.id} engineer={e} />)}
        </div>
      )}
    </div>
  );
